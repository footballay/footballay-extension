...
vi.mock('@/shared/footballayApiProtocol', () => ({ requestAvailableLeagues, requestFixtures }));
...
describe('match picker store', () => {
    ...
    it('selects a league and loads its fixtures in the same action flow', async () => {
        ...
        await useMatchPickerStore.getState().selectLeagueAndLoadFixtures('league-1');
        useMatchPickerStore.getState().selectFixture('fixture-1');

        expect(requestFixtures).toHaveBeenCalledWith(expect.objectContaining({
            leagueUid: 'league-1',
            mode: 'nearest',
    ...
    it('makes a fixture API failure available to the view', async () => {
        requestFixtures.mockResolvedValueOnce({ ok: false, error: 'Network unavailable' });
    ...
    it('ignores a fixture response from an earlier league selection', async () => {
        ...
        const firstSelection = useMatchPickerStore.getState().selectLeagueAndLoadFixtures('league-1');
        await useMatchPickerStore.getState().selectLeagueAndLoadFixtures('league-2');
        resolveFirstRequest({
            ok: true,
            data: [{
                uid: 'fixture-1',
